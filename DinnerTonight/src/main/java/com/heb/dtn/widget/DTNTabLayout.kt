...
import com.heb.dtn.widget.DTNTabLayout.Tab
import java.lang.ref.WeakReference
import java.util.*

/**
 * DTNTabLayout provides a horizontal layout to display tabs.
 *
 *
 ...
 * @attr ref R.styleable#DTNTabLayout_indicatorGravity
 */
class DTNTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : HorizontalScrollView(context, attrs, defStyleAttr) {
    ...
    private var mTabPaddingStart: Int = 0
    private var mTabPaddingTop: Int = 0
    private var mTabPaddingEnd: Int = 0
    private var mTabPaddingBottom: Int = 0

    private val mTabTextAppearance: Int
    private var mTabTextColors: ColorStateList? = null
    private var mTabTextSize: Float = 0.toFloat()
    private val mTabTextMultiLineSize: Float
    ...
    private val mIndicatorGravity: Int
    ...
    /**
     * Returns the current mode used by this [DTNTabLayout].
     *
     * @see .setTabMode
     */
    /**
    ...
    @IntDef(value = *longArrayOf(MODE_SCROLLABLE.toLong(), MODE_FIXED.toLong()))
    @Retention(AnnotationRetention.SOURCE)
    private annotation class Mode

    @IntDef(flag = true, value = *longArrayOf(GRAVITY_FILL.toLong(), GRAVITY_CENTER.toLong()))
    @Retention(AnnotationRetention.SOURCE)
    private annotation class TabGravity

    @IntDef(flag = true, value = *longArrayOf(TOP.toLong(), BOTTOM.toLong()))
    @Retention(AnnotationRetention.SOURCE)
    private annotation class IndicatorGravity
    ...
    init {
        ...
        super.addView(mTabStrip, 0, FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT))

        var a = context.obtainStyledAttributes(attrs, android.support.design.R.styleable.TabLayout,
        ...
        a = context.obtainStyledAttributes(attrs, R.styleable.DTNTabLayout)
        mIndicatorGravity = a.getInt(R.styleable.DTNTabLayout_indicatorGravity, BOTTOM)
        a.recycle()
    ...
    fun addTab(tab: Tab, position: Int, setSelected: Boolean = mTabs.isEmpty()) {
        if (tab.mParent !== this) {
            throw IllegalArgumentException("Tab belongs to a different DTNTabLayout.")
        }
    ...
    private fun addViewInternal(child: View) {
        if (child is TabItem) {
            ...
            throw IllegalArgumentException("Only TabItem instances can be added to DTNTabLayout")
        }
    }
    ...
    class Tab internal constructor()// Private constructor
    {
        ...
        internal var mParent: DTNTabLayout? = null
        ...
        val isSelected: Boolean
            get() {
                if (mParent == null) {
                    throw IllegalArgumentException("Tab not attached to a DTNTabLayout")
                }
    ...
    internal inner class TabView(context: Context) : LinearLayout(context), View.OnLongClickListener {
        var tab: Tab? = null
            set(tab) {
                if (tab != this.tab) {
                    field = tab
        ...
        fun update() {
            ...
            if (custom != null) {
                ...
                mCustomTextView = custom.findViewById(android.R.id.text1) as TextView
                if (mCustomTextView != null) {
                    mDefaultMaxLines = TextViewCompat.getMaxLines(mCustomTextView!!)
                }
                mCustomIconView = custom.findViewById(android.R.id.icon) as ImageView
            } else {
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    val iconView = LayoutInflater.from(context)
                            .inflate(android.support.design.R.layout.design_layout_tab_icon, this, false) as ImageView
                    addView(iconView, 0)
                    mIconView = iconView
                }
                if (mTextView == null) {
                    val textView = LayoutInflater.from(context)
                            .inflate(android.support.design.R.layout.design_layout_tab_text, this, false) as TextView
                    addView(textView)
                    mTextView = textView
        ...
        override fun onLongClick(v: View): Boolean {
            ...
            val cheatSheet = Toast.makeText(context, tab!!.getContentDescription(),
                    Toast.LENGTH_SHORT)
    ...
    private inner class SlidingTabStrip internal constructor(context: Context) : LinearLayout(context) {
        ...
        internal fun animateIndicatorToPosition(position: Int, duration: Int) {
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                ...
                animator?.addUpdateListener(ValueAnimator.AnimatorUpdateListener { animator ->
                    ...
                    setIndicatorPosition(lerp(startLeft.toFloat(), targetLeft.toFloat(), fraction).toInt(),
                            lerp(startRight.toFloat(), targetRight.toFloat(), fraction).toInt())
                })
        ...
        override fun draw(canvas: Canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                val top = (if (mIndicatorGravity == TOP) 0 else height - mSelectedIndicatorHeight).toFloat()
                val bottom = (if (mIndicatorGravity == TOP) mSelectedIndicatorHeight else height).toFloat()
                canvas.drawRect(mIndicatorLeft.toFloat(), top, mIndicatorRight.toFloat(), bottom, mSelectedIndicatorPaint)
            }
        }
    }
     ...
     * necessary calls back to the provided [DTNTabLayout] so that the tab position is
     * kept in sync.
     *
     *
     * This class stores the provided DTNTabLayout weakly, meaning that you can use
     * [ addOnPageChangeListener(OnPageChangeListener)][ViewPager.addOnPageChangeListener] without removing the listener and
     * not cause a leak.
     */
    class TabLayoutOnPageChangeListener(tabLayout: DTNTabLayout) : ViewPager.OnPageChangeListener {
        private val mTabLayoutRef: WeakReference<DTNTabLayout>
    ...
    companion object {
        ...
        /**
         * Gravity used to fill the [DTNTabLayout] as much as possible. This option only takes effect
        ...
        /**
         * Gravity used to lay out the tabs in the center of the [DTNTabLayout].
         *
         * @see .setTabGravity
         * @see .getTabGravity
         */
