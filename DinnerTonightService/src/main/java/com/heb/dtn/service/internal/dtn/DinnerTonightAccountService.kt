...
internal class DinnerTonightAccountService(config: HTTPService.Config) : HTTPService(config = config), AccountService {

    override fun getSessionToken(uid: UID, signature: String, timestamp: String): Promise<String> {
    ...
    override fun finalizeAccountRegistration(token: Registration, firstName: String, lastName: String): Promise<com.heb.dtn.service.domain.account.Session> {
